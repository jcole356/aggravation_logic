...
RSpec.describe 'Player' do
  ...
  describe 'Player#discard' do
    it 'removes on cards from the players hand' do
      expect_any_instance_of(Prompts).to receive(:card_discard_prompt).and_return(1)

      game.players << player1
      game.players << player2
      game.deal
      expect { player1.discard }.to change { player1.hand.cards.length }.by(-1)
    end
  end

  describe 'Player#swap' do
    let(:game) { build(:game) }
